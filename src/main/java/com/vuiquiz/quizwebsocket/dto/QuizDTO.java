...
public class QuizDTO {
    ...
    @Schema(description = "Visibility status (e.g., 1 for public, 0 for private)", example = "1", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
    ...
    @Schema(description = "URL (file_path) of the cover image for the quiz. Provided in responses after upload.",
            example = "https://your-server/files/images/unique_cover_key_123xyz.jpg", nullable = true)
    @JsonProperty("cover") // Matches mock field name for response
    private String cover; // This field will hold the URL (filePath) for responses.

    @Schema(description = "Unique key generated by the client to associate with the cover image file part in a multipart request. Used during quiz creation if a cover image is uploaded.",
            example = "cover_img_ref_1", nullable = true)
    @JsonProperty("coverImageUploadKey") // Ensure JSON key matches what frontend sends
    private String coverImageUploadKey; // New field
